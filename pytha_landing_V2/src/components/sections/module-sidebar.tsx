...
const MODULE_NAV = [
  ...
  { id: 'modul-cam', label: 'Interfață CAM', price: '1.950€', monthly: '44€' },
            ...
            <button
              ...
              className={cn(
                ...
                activeId === id
                  ...
                  : 'text-muted-foreground hover:bg-gray-50 hover:text-[#8a1820]',
              )}
            >
              {label}
              {cart.includes(id) && (
          ...
          <div className="space-y-2">
              ...
              <div key={m.id} className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="cursor-pointer rounded-full p-0.5 text-muted-foreground transition-colors hover:bg-red-50 hover:text-red-500"
                  >
                    <X className="size-3" />
                  </button>
                </div>
            ...
            <div className="mt-3 border-t border-border pt-3">
              <div className="flex items-baseline justify-between">
                ...
                <span className="text-lg font-bold text-[#8a1820]">{totalFormatted}<span className="text-xs font-normal text-muted-foreground">{totalLabel}</span></span>
              </div>
            </div>
        ...
        {mobileOpen && (
          <div className="mb-3 w-72 rounded-2xl border border-border bg-card p-4 shadow-xl">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button onClick={() => setMobileOpen(false)} className="cursor-pointer rounded-full p-1 text-muted-foreground hover:bg-gray-100">
                <X className="size-4" />
              </button>
            </div>
            {cart.length === 0 ? (
              <p className="text-xs text-muted-foreground">Adauga module apasand &ldquo;Include in oferta&rdquo;.</p>
            ) : (
              <>
                  ...
                  {cartModules.map((m) => (
                    <div key={m.id} className="flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-1">
                        <span className="text-muted-foreground">{isMonthly && m.monthly ? m.monthly : m.price}</span>
                ...
                <div className="mt-2 flex items-baseline justify-between border-t border-border pt-2">
                  ...
                  <span className="text-base font-bold text-[#8a1820]">{totalFormatted}<span className="text-[10px] font-normal text-muted-foreground">{totalLabel}</span></span>
                </div>
        ...
        <button
          ...
          className={cn(
            ...
            cart.length > 0
              ...
              : 'bg-card text-[#8a1820] border border-border hover:shadow-xl',
          )}
        >
          <ShoppingCart className="size-5" />
          {cart.length > 0 && (
            <span className="absolute -top-1 -right-1 flex size-5 items-center justify-center rounded-full bg-card text-[10px] font-bold text-[#8a1820] shadow">
              {cart.length}
            </span>
          )}
        </button>
